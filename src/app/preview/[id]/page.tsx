  ...
  return (
    <div className="min-h-screen bg-background">
      ...
      <div className="border-b border-border bg-card">
        <div className="max-w-7xl mx-auto px-4 py-4 sm:px-6 lg:px-8">
          ...
          <p className="text-sm text-muted-foreground mt-1">
            Views: {snippet.views} • Last updated: {snippet.updatedAt.toLocaleDateString()}
          </p>
          {snippet.expiresAt && (
            <p className="text-sm text-destructive mt-1">Expires: {snippet.expiresAt.toLocaleDateString()}</p>
          )}
        </div>
      </div>
      ...
      <div className="max-w-7xl mx-auto px-4 py-8 sm:px-6 lg:px-8">
        <HtmlPreviewClient html={snippet.html} css={snippet.css || ""} js={snippet.js || ""} />
      </div>
    </div>
  )
}
